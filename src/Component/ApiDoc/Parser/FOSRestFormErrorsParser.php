...
// on étend le parser pour en créer un qui soit perso
// il permet de générer les erreurs de validation au même format que FOSRestBundle.
// voir doc sur parser
...
class FOSRestFormErrorsParser implements ParserInterface, PostParserInterface
{
    ...
    public function postParse(array $item, array $parameters)
    {
        ...
        foreach ($parameters as $name => $parameter) {
            $params['errors']['children']['children']['children'][$name] = $this->doPostParse($parameter, $name,
                [$name], $item['class']);
        }
    ...
    protected function doPostParse($parameter, $name, array $propertyPath, $type)
    {
        ...
        if ($parameter['actualType'] == DataTypes::COLLECTION) {
            ...
            foreach ($parameter['children'] as $cName => $cParameter) {
                ...
                $data['children']['children']['children']['children']['children'][$cName] = $this->doPostParse($cParameter,
                    $cName, $cPropertyPath, $parameter['subType']);
            }
